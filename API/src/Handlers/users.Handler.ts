...
export const createUser = async (
  ...
  try {
    const response = await userModel.CreateUser(req.body as User);
...
export const getUsers = async (req: express.Request, res: express.Response) => {
  try {
    const response = await userModel.GetUsers(req.body.username as string);
    res.status(200).json({
      status: "SUCCESS",
      users: response,
...
export const authenticate = async (
  ...
  try {
    ...
    const response = await userModel.Authenticate(user.email as string, user.password as string);
    if (response !== null) {
      const token = generateToken(response);
      if (!token) {
